...
class BlockStore(rpyc.Service):
    ...
    def __init__(self):
        self.hash_blocks = {}        
        logging.basicConfig(filename='blockstore.log', level=logging.DEBUG, format='%(asctime)s  %(message)s', datefmt='%m%d%Y %I:%M:%S %p')
    ...
    def exposed_store_block(self, h, block):        
        logging.info("store block: " + str(h))
